...
import { fetchTransactionDetail, fetchCaseExplanation } from "../services/api.js";
...
export function CaseDetail({ txnId, onBack }: CaseDetailProps) {
  ...
  const handleExplain = async () => {
    try {
      ...
      const res = await fetchCaseExplanation(txnId);
      setExplanation(res.explanation);
      if (res.source) {
        setExplainSource(res.source);
      }
    } catch (err) {
      setExplainError(
        err instanceof Error
          ? err.message
          : "Failed to generate plain-English explanation"
      );
    } finally {
      setExplaining(false);
    }
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        ...
        <div className="flex items-center gap-3 self-start sm:self-auto">
          {/* Phase 6: Explain This Case Button */}
          <button
            onClick={handleExplain}
            disabled={explaining}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-500 hover:to-pink-500 text-white text-xs font-semibold shadow-lg shadow-indigo-600/30 transition cursor-pointer disabled:opacity-50"
          >
            <SparklesIcon className={`w-4 h-4 ${explaining ? "animate-spin" : ""}`} />
            {explaining ? "Explaining Case..." : "Explain this case"}
          </button>

          <button
            onClick={loadDetail}
            disabled={loading}
            className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 text-xs font-medium transition cursor-pointer"
          >
            <RefreshIcon
      ...
      {/* ======================================================== */}
      {/* PHASE 6: LLM EXPLAINABILITY LAYER CARD                  */}
      {/* ======================================================== */}
      {(explanation || explaining || explainError) && (
        <div className="p-6 rounded-2xl bg-gradient-to-r from-indigo-950/50 via-purple-950/40 to-slate-900/90 border border-indigo-500/40 shadow-2xl space-y-3 relative overflow-hidden animate-fadeIn">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-indigo-800/40 pb-3">
            <div className="flex items-center gap-2">
              <span className="p-1 rounded-md bg-indigo-900/70 border border-indigo-600/50 text-indigo-300">
                <SparklesIcon className="w-4 h-4 text-indigo-300" />
              </span>
              <h2 className="text-sm font-bold text-white tracking-wide">
                Merchant Plain-English Case Summary
              </h2>
            </div>
            <div className="flex items-center gap-2">
              {explainSource && (
                <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-purple-950/80 text-purple-300 border border-purple-700/40">
                  {explainSource === "anthropic-claude" ? "Claude 3.5 Haiku" : "Deterministic Fallback"}
                </span>
              )}
              <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-indigo-950/80 text-indigo-300 border border-indigo-700/50">
                Narrative Layer
              </span>
            </div>
          </div>
          ...
          {explaining && (
            <div className="py-4 flex items-center gap-3 text-slate-300 text-xs">
              <div className="w-4 h-4 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin"></div>
              <span>Synthesizing audit trail events into merchant-friendly explanation...</span>
            </div>
          )}
          ...
          {explanation && (
            <div className="space-y-3 pt-1">
              <p className="text-sm text-slate-100 font-sans leading-relaxed bg-slate-900/60 p-4 rounded-xl border border-indigo-500/20 shadow-inner">
                {explanation}
              </p>
              <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1">
                <span>
                  Strictly Observational: Explains past audit logs; does not alter retry execution.
                </span>
                <button
                  onClick={handleExplain}
                  disabled={explaining}
